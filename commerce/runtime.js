    ...
    const response = await request('https://api.sendgrid.com/v3/mail/send', {
      ...
      body: JSON.stringify({
        ...
        content: [{ type: 'text/plain', value: [
          ...
          ...(release.channel === 'beta' ? ['This is the signed, notarized beta for Apple silicon Macs. It includes Codex Vault search, encrypted versioned backup, recovery, and guided Mac-to-Mac migration. Keep your old Mac and an independent backup until you verify the result. Current beta limits: https://migrate.segeren.com/#founding-edition'] : []),
          '%closing%',
        ].join('\n\n') }],
    ...
    const response = await request('https://api.sendgrid.com/v3/mail/send', {
      ...
      body: JSON.stringify({
        ...
        content: [{ type: 'text/plain', value: [
          ...
          `Release: ${release.id}`,
          'Codex Migrate includes Codex Vault for local conversation search, encrypted versioned backup, and guarded recovery, plus guided Mac-to-Mac migration. The signed and Apple-notarized Apple-silicon beta is $49 once, with best-effort support and a 30-day refund policy.',
