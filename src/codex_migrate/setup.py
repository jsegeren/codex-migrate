...
<h1>Change the Mac. Keep the work.</h1><p>Set up your Mac-to-Mac migration here. This page is served by the helper on your old Mac. Your workspace travels directly between your Macs over SSH—not through our website.</p>
...
<details><summary>Connection and permissions help</summary><p>A supported USB-C/Thunderbolt network connection can carry the transfer; a charging-only cable cannot. After verifying the configured Mac, inspection safely checks its reachable private network addresses and selects the fastest verified route. Set up SSH key login from the old Mac and verify the new Mac’s host fingerprint through a trusted channel. Connect once in Terminal using <code>ssh new-user@new-mac.local</code>. We never disable host verification. The helper cannot answer an interactive password prompt.</p></details>
