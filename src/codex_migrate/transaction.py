...
def recovery_preflight_script(home):
    return ("/usr/bin/perl -MJSON::PP -MIO::Handle -e 'IO::Handle->can(\"sync\") or exit 78' "
...
$mode =~ /\A(?:begin|installed|restored|clear)\z/ or fail();
my $plan = eval { JSON::PP->new->utf8->decode($payload) };
!$@ && ref($plan) eq 'HASH' && $plan->{format} == 1 &&
...
if ($mode eq 'begin') {
    ...
    $record->{id} eq $id && $record->{home} eq $home && $record->{backup} eq $backup &&
...
def transaction_commands(home, backup, mappings):
    ...
    payload = json.dumps({"format": 1, "id": secrets.token_hex(16), "home": home,
                          "backup": backup,
