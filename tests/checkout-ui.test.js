...
test('beta availability keeps the beta state visible while the CTA leads with the outcome', async () => {
  ...
  assert.equal(f.get('checkout-button').textContent, 'Protect my Codex work — $49');
  assert.match(f.get('edition-state').textContent, /beta/);
  assert.equal(f.get('hero-availability').textContent, 'Signed & notarized Mac beta · Codex Vault included');
});
