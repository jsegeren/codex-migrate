        ...
        do {
            ...
            child.terminationHandler = { child in
                ...
                DispatchQueue.main.async {
                    ...
                    if child.terminationStatus != 0 && child.terminationReason == .exit {
...
final class AppDelegate: NSObject, NSApplicationDelegate {
    func applicationShouldTerminate(_ sender: NSApplication) -> NSApplication.TerminateReply {
        if MigrationModel.shared.running {
            ...
            alert.informativeText = "Use Stop safely in the dashboard, then Close dashboard here before quitting. During installation, wait for verification to finish."
            alert.runModal()
                    ...
                    GroupBox("2 · Destination and scope") {
                        VStack(alignment: .leading, spacing: 12) {
                            ...
                            ForEach(model.workspaces, id: \.self) { path in
                                HStack { Text(path).textSelection(.enabled); Spacer(); Button("Remove") { model.workspaces.removeAll { $0 == path } } }
                            }
