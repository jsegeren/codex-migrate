(() => {
  ...
  fetch('/api/availability', { credentials: 'same-origin', cache: 'no-store', signal: AbortSignal.timeout(8000) })
    .then(async response => {
      ...
      document.getElementById('checkout-platform').textContent = `For ${platform}. $49 USD plus applicable tax. Secure checkout through Stripe.`;
      ...
      document.getElementById('hero-availability').textContent = beta ? 'Signed & notarized Mac beta · Codex Vault included' : 'Mac app available';
      ...
      button.textContent = 'Protect my Codex work — $49';
      panel.hidden = false;
