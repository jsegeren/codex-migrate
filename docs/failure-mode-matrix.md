...
| Power lost during replacement or rollback | Destination recovery record is synced before removal; pending or malformed records block staging/installation writes. Normal rollback now receives content/absence verification and a synced terminal receipt. Real local SIGKILL fixtures cover full/skills removal; injected flush, failed rollback and terminal-cleanup failures are tested. | Guided post-crash restore/reconciliation remains required, along with later replacement boundaries and real hardware/second-Mac failure acceptance. Do not equate safe containment or a local process exit with completed recovery. |
