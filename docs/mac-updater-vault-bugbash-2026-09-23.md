...
| --- | --- | --- |
| Python regression | Current candidate ran 838 tests: 825 passed, 13 skipped, including the new native install-location check and existing notarization/rotation resume fixtures. | Release/notarization results printed by mocked fixture tests are not real Apple receipts. Opt-in physical filesystem and clean-account tests are separate; a green suite is not buyer acceptance. |
...
| External Vault disconnect | A synthetic Vault on a real detachable APFS disk image survived eject and remount. While detached, the scheduled runner failed without creating a replacement Vault on the internal volume; status showed unhealthy. After remount, the next scheduled runner capture verified successfully. The schedule is now bound to the verified Vault key ID; a changed destination fails closed. | This exercised the real scheduler runner, not the account-wide LaunchAgent or a physical USB drive. Older schedules without a bound Vault identity stop and instruct the user to turn scheduling off and on again; that UI path still needs a rendered check. |
