...
| Backup scheduling | Found that a loaded schedule could look healthy with a days-old last success or stalled run; candidate now marks overdue runs unhealthy and disregards receipts from before reinstallation. A real macOS LaunchAgent ran the packaged engine against a disposable Codex home and Vault, produced a second verified snapshot, and was unloaded afterward. This exposed a Keychain failure when the Codex source folder differed from the signed-in account's home; the generated agent now sets `HOME` to the account home while preserving the explicit source path. The corrected LaunchAgent run passed. | The agent was triggered with `launchctl kickstart`, not by waiting through a 24-hour interval or a sleep/restart cycle. The fixture did not use a buyer account's real Codex history. |
