...
| Updater preferences | Review found that Sparkle's automatic download normally installs on quit. The next source candidate adds a private idle probe and an opt-in quit/retry path, with a final shutdown recheck; Swift typecheck, 4 focused endpoint tests, and 9 packaged dashboard tests pass (1 filesystem skip). A running scheduled backup now refuses the idle probe and shutdown. | The existing notarized build-17 ZIP predates this change. Rebuild, notarize, and physically verify unattended installation, including a separately scheduled Vault backup, before release or marketing claims. |
